...
function MessageForm({ onSuccess, onMessageSent }: MessageFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await fetch(
        "https://docs.google.com/forms/d/e/1FAIpQLSejlkYrs7M9QNKL4hEYnb5psuxBCANXKTgc2Yqnog4KG0rmAg/formResponse",
        {
          method: "POST",
          mode: "no-cors",
          body: googleFormData,
  ...
  return (
    <div className="relative w-full max-w-md sm:max-w-lg mx-auto px-2 sm:px-3 md:px-0">
      ...
      <div className="hidden sm:block absolute -top-4 -left-4 w-8 h-8 bg-[#869EB6]/20 rounded-full blur-sm animate-pulse sm:w-12 sm:h-12 sm:-top-6 sm:-left-6"></div>
      <div className="hidden sm:block absolute -bottom-4 -right-4 w-12 h-12 bg-[#83927D]/15 rounded-full blur-md animate-pulse sm:w-16 sm:h-16 sm:-bottom-6 sm:-right-6"></div>
      
      <Card className={`relative w-full border-2 shadow-[0_8px_32px_rgba(134,158,182,0.18)] bg-white/95 backdrop-blur-md transition-all duration-500 group overflow-hidden rounded-xl sm:rounded-2xl ${
        isFocused ? 'scale-[1.02] border-[#869EB6] bg-white' : 'border-[#869EB6]/40 hover:border-[#869EB6]/60 hover:bg-white'
      } ${isSubmitted ? 'animate-bounce' : ''}`}>
        {/* Subtle background overlay */}
        <div className="absolute inset-0 bg-[#869EB6]/5"></div>
        
        {/* Animated shine effect */}
        <div className="absolute inset-0 bg-[#869EB6]/10 -translate-x-full group-hover:translate-x-full transition-transform duration-1000"></div>
        
        {/* Success animation overlay */}
        {isSubmitted && (
          <div className="absolute inset-0 bg-[#83927D]/20 flex items-center justify-center z-20 pointer-events-none">
            <div className="flex flex-col items-center gap-2 animate-pulse">
              <div className="w-12 h-12 sm:w-16 sm:h-16 bg-[#83927D] rounded-full flex items-center justify-center shadow-lg">
                <Sparkles className="h-6 w-6 sm:h-8 sm:w-8 text-white" />
              </div>
              <p className="text-[#83927D] font-semibold text-sm sm:text-lg">Sent!</p>
            </div>
          </div>
        )}
        ...
        <CardContent className="relative p-4 sm:p-5 md:p-7 lg:p-9">
          ...
          <div className="text-center mb-4 sm:mb-6 md:mb-8">
            <div className="relative inline-block mb-2 sm:mb-3 md:mb-4">
              <div className="absolute inset-0 bg-[#869EB6]/20 rounded-full blur-lg scale-150"></div>
              <div className="relative w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 bg-[#869EB6] rounded-full flex items-center justify-center mx-auto shadow-lg">
          ...
          >
            ...
            <div className="space-y-1 sm:space-y-1.5 md:space-y-3">
              <label className="block text-[11px] sm:text-xs md:text-sm font-medium text-foreground font-lora flex items-center gap-1.5 sm:gap-2">
                <div className={`w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 bg-[#83927D]/20 rounded-full flex items-center justify-center transition-all duration-300 ${
                  focusedField === 'name' ? 'scale-110 bg-[#83927D]/30' : ''
                }`}>
                  <Heart className="h-2.5 w-2.5 sm:h-3 sm:w-3 md:h-4 md:w-4 text-[#83927D]" />
                </div>
                Your Name
              </label>
              <div className="relative">
                <Input
                  ...
                  className={`w-full border-2 rounded-lg sm:rounded-xl py-2 sm:py-2.5 md:py-3.5 px-3 sm:px-3.5 md:px-5 text-xs sm:text-sm md:text-base font-lora placeholder:text-foreground/50 transition-all duration-300 bg-white backdrop-blur-sm shadow-sm hover:shadow-md focus:shadow-lg ${
                    focusedField === 'name' 
                      ? 'border-[#869EB6] focus:border-[#869EB6] focus:ring-2 sm:focus:ring-4 focus:ring-[#869EB6]/20 shadow-lg' 
                      : 'border-[#869EB6]/30 hover:border-[#869EB6]/50'
                  }`}
                />
                {nameValue && (
                  <div className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2">
                    <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-[#83927D] rounded-full animate-pulse"></div>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-1 sm:space-y-1.5 md:space-y-3">
              <div className="flex items-center justify-between">
                <label className="block text-[11px] sm:text-xs md:text-sm font-medium text-foreground font-lora flex items-center gap-1.5 sm:gap-2">
                  <div className={`w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 bg-[#83927D]/20 rounded-full flex items-center justify-center transition-all duration-300 ${
                    focusedField === 'message' ? 'scale-110 bg-[#83927D]/30' : ''
                  }`}>
                    <MessageCircle className="h-2.5 w-2.5 sm:h-3 sm:w-3 md:h-4 md:w-4 text-[#83927D]" />
                  </div>
                  Your Message
                </label>
                {messageValue && (
            ...
            <Button
              ...
              className="w-full bg-[#83927D] hover:bg-[#83927D]/90 text-[#FFFFFF] py-2 sm:py-2.5 md:py-4 px-4 sm:px-5 md:px-8 rounded-lg sm:rounded-xl text-xs sm:text-sm md:text-base font-lora font-semibold shadow-lg backdrop-blur-sm transition-all duration-300 hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none relative overflow-hidden group border border-[#869EB6]/30"
              style={{ backgroundColor: '#83927D' }}
              onMouseEnter={(e) => {
                e.currentTarget.style.backgroundColor = '#83927D'
...
export function Messages() {
  ...
  const fetchMessages = useCallback(() => {
    setLoading(true)
    fetch(
      "https://script.google.com/macros/s/AKfycbzets7SMhaX5W4NRgK_eBUEfSBhsOopWkx44mSbGiqWXq_4tC69j8Ml1rshem6dLmUF-A/exec"
    )
      .then((res) => res.json())
      .then((data) => {
    ...
    <Section id="messages" bgColor="sand">
      <div className="relative max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden" aria-hidden="true">
          ...
          <div className="hidden sm:block absolute -top-6 -left-6 w-24 h-24 bg-[#869EB6]/10 rounded-full blur-2xl animate-pulse" />
          <div className="hidden sm:block absolute top-10 right-0 w-20 h-20 bg-[#83927D]/15 rounded-full blur-xl animate-pulse delay-1000" />
          <div className="hidden sm:block absolute bottom-10 left-10 w-28 h-28 bg-[#869EB6]/8 rounded-full blur-2xl animate-pulse delay-2000" />

          {/* Gradient lines - hidden on mobile */}
          <div className="hidden sm:block absolute top-1/3 left-0 w-full h-px bg-[#869EB6]/25" />
          <div className="hidden sm:block absolute bottom-1/4 left-0 w-full h-px bg-[#83927D]/20" />
        </div>
        ...
        <div className="flex justify-center mb-6 sm:mb-8 md:mb-12 lg:mb-16">
          <div className="relative max-w-2xl w-full">
            ...
            <div className="hidden sm:block absolute -inset-3 bg-[#869EB6]/15 rounded-3xl blur-2xl opacity-70" />
            <div className="hidden sm:block absolute -inset-1 bg-[#83927D]/10 rounded-3xl blur-md opacity-80" />
        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          <div className="hidden sm:block absolute -top-3 -left-3 w-4 h-4 bg-[#869EB6] rounded-full blur-sm opacity-70" />
          <div className="hidden sm:block absolute -top-3 -right-3 w-4 h-4 bg-[#83927D] rounded-full blur-sm opacity-70" />
          
          <div className="text-center mb-5 sm:mb-6 md:mb-8 lg:mb-10">
            <div className="relative inline-block mb-2 sm:mb-3 md:mb-4 lg:mb-5">
              <div className="absolute inset-0 bg-[#869EB6]/20 rounded-full blur-xl scale-150"></div>
              <div className="relative w-9 h-9 sm:w-10 sm:h-10 md:w-12 md:h-12 lg:w-14 lg:h-14 bg-[#869EB6] rounded-full flex items-center justify-center mx-auto shadow-lg">
